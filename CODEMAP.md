...
- `internal/db/`: sqlite store. Queries in `sqlite/*.sql`, migrations in `sqlite/migrations/`. `lewkit generate db internal/db` writes sqlc into `sqlite/` plus `zz_fs.go` / `zz_queries.go`.
