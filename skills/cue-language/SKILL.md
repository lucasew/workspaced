...
If the task is **only** workspaced product/ops (plan/apply, lock/tidy, config
roots, modules enablement, templates, tools, drivers) and not “why won’t this
CUE unify / type / default?”, prefer **`skills/workspaced/`** and **skip this
skill** — even when the file is `workspaced.cue`.

If **both** apply (non-trivial CUE authoring for workspaced):

1. This skill — legal CUE (unify, defs, constraints, examples).
2. **`skills/workspaced/references/cue.md`** + **`modules.md`** — evaluated keys,
   merge edges, lock vs cue, inspect via workspaced config commands.
3. Always read the user’s **actual** `workspaced.cue` (and modules); init/examples
   in either skill are not the local dialect.
...
- Keep **merge-friendly shapes** at composition boundaries: keyed structs over
  naked lists when layers/overlays unify. For *workspaced* specifically (module
  `config` merge physics, not a global CUE ban on lists):
...
- Workspaced product/ops (apply, lock, roots, modules, tools, drivers, templates):
  **`skills/workspaced/`** only — load this skill only if language/unification
  is also blocking.
...
- CLI (`cue eval`, `cue export`, …): run the tool; this skill is language semantics.
  Workspaced may use different loaders/preludes than bare `cue eval` — product
  inspect: `workspaced home|codebase config …` (`--help`).
