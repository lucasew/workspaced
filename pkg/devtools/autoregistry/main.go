...
func HandleRegistryCodegen(r DetectedRoot) error {
	...
	_, _ = fmt.Fprintf(f, "\n")
	_, _ = fmt.Fprintf(f, "// this file is generated by pkg/devtools/autoregistry.go, do not edit manually\n")
	_, _ = fmt.Fprintf(f, "import (\n")
	for _, c := range children {
		_, _ = fmt.Fprintf(f, "\tpkg_%s \"%s\"\n", c.Package, c.ImportPath)
	}
	_, _ = fmt.Fprintf(f, ")\n")
	_, _ = fmt.Fprintf(f, "\n")
	_, _ = fmt.Fprintf(f, "func init() {\n")
	for _, c := range children {
		_, _ = fmt.Fprintf(f, "\tRegistry.FromGetter(pkg_%s.GetCommand)\n", c.Package)
	}
	_, _ = fmt.Fprintf(f, "}\n")
	for _, c := range children {
